...
**IMPORTANT**: Follow ALL steps. The build system regenerates admin_html.h automatically on each build, but you MUST verify it worked.

```bash
# 1. Edit the source file
#    data/admin.html

# 2. Bump version in config.h (REQUIRED for device to reprovision)
#    #define FIRMWARE_VERSION "1.4.9"

# 3. Build (the pre-script auto-generates admin_html.h)
/Users/ryanmaule/Library/Python/3.9/bin/pio run -e esp8266

# 4. VERIFY the build output shows admin_html generation at the START:
#    [admin_html] Generating .../src/admin_html.h
#    [admin_html] Source: .../data/admin.html
#    [admin_html] Version: 1.4.9   <-- Should match your new version!
#    [admin_html] Original: XXXXX bytes
#    [admin_html] Compressed: XXXXX bytes
#
#    If you don't see this, or see it at the END of build output, something is wrong.

# 5. Flash via OTA
curl -F "firmware=@.pio/build/esp8266/firmware.bin" http://192.168.4.235/update

# 6. Wait for reboot (~20 seconds), then verify:
curl -s --compressed http://192.168.4.235/admin | head -20
# Should show your new HTML content
```

**If changes don't appear after flashing**:
1. Force reprovision: `curl http://192.168.4.235/api/reprovision`
2. Wait 15 seconds, hard refresh browser (Cmd+Shift+R)
